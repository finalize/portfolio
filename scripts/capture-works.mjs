...
const SOURCES = {
  ...
  // mimawari は CLI なので撮る画面が無い。カード画像を1枚描いて元にする。
  // 端末出力そのものは文字が細かすぎて、64マスに落とすとただの灰色の面になる
  // （太い色面と 60px 以上の文字だけが残る、という前提でこの構図にしてある）。
  mimawari: { file: 'scripts/mimawari-card.svg' },
